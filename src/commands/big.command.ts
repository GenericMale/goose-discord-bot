...
export class BigCommand extends Command {
    ...
    async execute(options, author): Promise<CommandResponse> {
        if (options.user) {
    ...
    private async executeEmoji(emoji: string, emojiSize: number): Promise<CommandResponse> {
        ...
        if (customMatch) {
            return {
                files: [{
                    attachment: `https://cdn.discordapp.com/emojis/${customMatch[3]}.${customMatch[1] ? 'gif' : 'png'}`,
                    name: `${customMatch[2]}.${customMatch[1] ? 'gif' : 'png'}`,
                }],
            };
        }
